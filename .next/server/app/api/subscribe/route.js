"use strict";(()=>{var e={};e.id=925,e.ids=[925],e.modules={20399:e=>{e.exports=require("next/dist/compiled/next-server/app-page.runtime.prod.js")},30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},84770:e=>{e.exports=require("crypto")},9553:(e,t,o)=>{o.r(t),o.d(t,{originalPathname:()=>y,patchFetch:()=>b,requestAsyncStorage:()=>f,routeModule:()=>m,serverHooks:()=>u,staticGenerationAsyncStorage:()=>g});var a={};o.r(a),o.d(a,{POST:()=>d});var i=o(49303),r=o(88716),s=o(60670),n=o(87070),l=o(36119);let p=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;async function c(e){return(await Promise.resolve().then(o.t.bind(o,84770,23))).createHash("md5").update(e.toLowerCase()).digest("hex")}async function d(e){try{let t;let o=await e.json(),a=(o.email||"").trim(),i=(o.firstName||"").trim(),r=(o.interest||"").trim(),s=(o.source||"Website Popup").trim();if(!a||!p.test(a))return n.NextResponse.json({error:"Please enter a valid email address."},{status:400});let d=process.env.MAILCHIMP_API_KEY,m=process.env.MAILCHIMP_AUDIENCE_ID,f=process.env.MAILCHIMP_API_SERVER||process.env.MAILCHIMP_SERVER_PREFIX,g=d?`Basic ${Buffer.from(`anystring:${d}`).toString("base64")}`:"",u=["Website Popup","Aivara Lead"];r&&u.push(r);let y=!1;if(d&&m&&f)try{let e=await c(a),t=await fetch(`https://${f}.api.mailchimp.com/3.0/lists/${m}/members/${e}`,{method:"PUT",headers:{Authorization:g,"Content-Type":"application/json"},body:JSON.stringify({email_address:a,status_if_new:"subscribed",merge_fields:{FNAME:i||"",INTEREST:r||"",SOURCE:s,COMPANY:"Aivara Solutions Website"}})});if(t.ok)y=!0,await fetch(`https://${f}.api.mailchimp.com/3.0/lists/${m}/members/${e}/tags`,{method:"POST",headers:{Authorization:g,"Content-Type":"application/json"},body:JSON.stringify({tags:u.map(e=>({name:e,status:"active"}))})});else{let e=await t.json().catch(()=>({}));console.error("Mailchimp upsert failed:",t.status,e),y=!1}}catch(e){console.error("Mailchimp request error:",e)}else console.warn("Mailchimp env vars not fully configured — skipping list signup");let b=!0;try{await (0,l.Pi)({firstName:i,email:a,interest:r})}catch(e){b=!1,t=e instanceof Error?e.message:String(e),console.error("[subscribe] welcome email failed:",t)}try{await (0,l.dc)(a)}catch(e){console.error("[subscribe] admin notification failed:",e)}return n.NextResponse.json({message:"You're in! Check your inbox for a welcome message from Aivara Solutions.",mailchimp:y,welcome:b,...t?{welcomeError:t}:{}},{status:200})}catch(e){return console.error("Subscription error:",e),n.NextResponse.json({error:"Something went wrong. Please try again or contact us directly."},{status:500})}}let m=new i.AppRouteRouteModule({definition:{kind:r.x.APP_ROUTE,page:"/api/subscribe/route",pathname:"/api/subscribe",filename:"route",bundlePath:"app/api/subscribe/route"},resolvedPagePath:"/home/runner/workspace/app/api/subscribe/route.ts",nextConfigOutput:"standalone",userland:a}),{requestAsyncStorage:f,staticGenerationAsyncStorage:g,serverHooks:u}=m,y="/api/subscribe/route";function b(){return(0,s.patchFetch)({serverHooks:u,staticGenerationAsyncStorage:g})}},36119:(e,t,o)=>{o.d(t,{Lf:()=>d,Pi:()=>m,dc:()=>f});var a=o(82591);let i=process.env.NOTIFICATION_EMAIL||"Kevin@AivaraSolutions.com",r=process.env.NOTIFICATION_FROM||"Aivara Solutions <notifications@aivarasolutions.com>",s="https://aivarasolutions.com";function n(){let e=process.env.RESEND_API_KEY;return e?new a.R(e):null}function l(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function p(){return new Date().toLocaleString("en-US",{timeZone:"America/Los_Angeles",weekday:"short",month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})+" PST"}function c(e){return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="background-color:#f4f4f7;">
        ...
        <tr>
          <td style="background-color:#fafafa; border-top:1px solid #ececec; padding:24px 40px; text-align:center;">
            ...
            <p style="margin:0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; font-size:11px; color:#9ca3af;">
              <a href="${s}" style="color:#14b8a6; text-decoration:none;">aivarasolutions.com</a>
              &nbsp;\xb7&nbsp;
...
</html>`}async function d(e){let t=n();if(!t){console.warn("RESEND_API_KEY not set — skipping notification email");return}let o=e.source||"Contact Form",a=`New Lead \xb7 ${e.name} \xb7 ${o}`,s=p(),d=(e,t,o)=>{let a=l(t),i="email"===o?`<a href="mailto:${a}" style="color:#0f766e; text-decoration:none; font-weight:500;">${a}</a>`:"tel"===o?`<a href="tel:${a}" style="color:#0f766e; text-decoration:none; font-weight:500;">${a}</a>`:`<span style="color:#1a1a1a; font-weight:500;">${a}</span>`;return`
      <tr>
  ...
  `,f=c({preheader:`New ${o} lead from ${e.name} — ${e.service}`,badge:"NEW LEAD",title:"New Lead Inquiry",intro:`${e.name} just reached out via ${o} (${e.service}).`,bodyHtml:m}),g=`AIVARA SOLUTIONS — New Lead Inquiry
${"-".repeat(50)}
...
Aivara Solutions \xb7 aivarasolutions.com`;try{let o=await t.emails.send({from:r,to:i,replyTo:e.email,subject:a,html:f,text:g});o.error&&console.error("[Resend] contact notification API error:",o.error)}catch(e){console.error("[Resend] contact notification failed:",e)}}async function m(e){let t=n();if(!t){console.warn("RESEND_API_KEY not set — skipping welcome email");return}let o=(e.firstName||"").trim()||"there",a=["AI Automation Systems","Client Portals & Business Dashboards","Lead Generation & Marketing Automation","AI Training & Business Onboarding","Websites + Automation Buildouts","Creative Content & Brand Media"].map(e=>`
      <tr>
    ...
    <p style="margin:0 0 28px 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; font-size:14px; color:#4b5563;">
      Or
      <a href="${s}" style="color:#0f766e; text-decoration:none; font-weight:600;">explore Aivara Solutions →</a>
    </p>
    ...
    <p style="margin:24px 0 6px 0; padding-top:20px; border-top:1px solid #ececec; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; font-size:13px; color:#6b7280; line-height:1.6;">
      ...
      <a href="${s}" style="color:#0f766e; text-decoration:none;">aivarasolutions.com</a>
    </p>
...
Book a Free Strategy Call: ${s}/contact
Explore Aivara Solutions: ${s}

Kevin \xb7 Aivara Solutions
kevin@aivarasolutions.com \xb7 310-400-0032
${s}

Aivara Solutions — AI-powered systems for modern businesses.`;try{let o=await t.emails.send({from:r,to:e.email,replyTo:"kevin@aivarasolutions.com",subject:"Welcome to Aivara Solutions — Build, Automate & Scale Smarter",html:i,text:p});if(o.error)throw console.error("[Resend] welcome email API error:",o.error),Error(`Resend rejected welcome email: ${JSON.stringify(o.error)}`);console.log("[Resend] welcome email sent to",e.email,"id:",o.data?.id)}catch(e){throw console.error("[Resend] welcome email failed:",e),e}}async function f(e){let t=n();if(!t){console.warn("RESEND_API_KEY not set — skipping notification email");return}let o=p(),a=`
    <p style="margin:0 0 20px 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Arial, sans-serif; font-size:14px; color:#4b5563; line-height:1.6;">
  ...
  `,s=c({preheader:`New newsletter subscriber: ${e}`,badge:"NEW SUBSCRIBER",title:"New Newsletter Signup",intro:`${e} just subscribed to your newsletter.`,bodyHtml:a}),d=`AIVARA SOLUTIONS — New Newsletter Subscriber
${"-".repeat(50)}
...
Aivara Solutions \xb7 aivarasolutions.com`;try{let o=await t.emails.send({from:r,to:i,replyTo:e,subject:`New Newsletter Subscriber \xb7 ${e}`,html:s,text:d});o.error&&console.error("[Resend] newsletter notification API error:",o.error)}catch(e){console.error("[Resend] newsletter notification failed:",e)}}}};var t=require("../../../webpack-runtime.js");t.C(e);var o=e=>t(t.s=e),a=t.X(0,[276,475],()=>o(9553));module.exports=a})();